...
execute unless score @s plushpets.loveID matches 1.. if entity @e[type=minecraft:armor_stand,tag=plushpets.breeding_partner] run function plushpets.1:init_breed_partners

#get pet's ID
execute if score @s plushpets.loveID matches 1.. run scoreboard players operation #love_ID sourcecraft.temp = @s plushpets.loveID

#if player matches ID, tag them
execute if score @s plushpets.loveID matches 1.. as @e[type=minecraft:armor_stand,tag=plushpets.plush_pet,tag=!plushpets.self] if score @s plushpets.loveID = #love_ID sourcecraft.temp run tag @s add plushpets.breed_partner_found
...
#when close, breed
execute if score @s plushpets.loveID matches 1.. if entity @e[type=minecraft:armor_stand,tag=plushpets.breed_partner_found,distance=0.01..1,tag=!plushpets.breed_init] run function plushpets.1:init_plush_pet_breed

#reset
tag @e[type=minecraft:armor_stand,tag=plushpets.breed_partner_found] remove plushpets.breed_partner_found
scoreboard players reset #love_ID sourcecraft.temp
